...
def send_email(count_ebs_excluidos_por_regiao: int, count_ebs_nao_excluidos_por_regiao: int) -> None:
    ...
    message = "Olá,\n\n"
    message += "A função Lambda para exclusão de EBS unattached foi executada com sucesso!\n\n"
    ...
    for regiao, count_excluidos in count_ebs_excluidos_por_regiao:
        count_nao_excluidos = next(
            (count for reg, count in count_ebs_nao_excluidos_por_regiao if reg == regiao),
            0,
        )
        message += f"{regiao}\t\t{count_excluidos}\t\t{count_nao_excluidos}\n"
    message += "--------------------------------------------------\n"
    message += f"""Total de EBS verificados: {sum([count for _, count in count_ebs_excluidos_por_regiao]) +
                                              sum([count for _, count in count_ebs_nao_excluidos_por_regiao])}.\n"""
            ...
            for volume in page["Volumes"]:
                ...
                if not volume["Attachments"]:
                    ...
                    if "VolumeState" in tags and tags["VolumeState"].lower() == "inuse":
                        print(f"Volume {volume['VolumeId']} não pode ser excluído pois está marcado como em uso")
